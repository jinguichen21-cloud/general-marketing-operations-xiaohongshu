...
```
# OpenClaw 示例
<openclaw-project>/skills/xiaohongshu/

# Claude Code 示例
<your-project>/.claude/skills/xiaohongshu/
```
...
# 克隆项目
git clone https://github.com/anthropics/xiaohongshu.git
cd xiaohongshu
...
若 xxClaw 同时加载了 **browseruse**（通用浏览器自动化）和本技能，用户请求「小红书相关操作」可能被错误路由到 browseruse，导致登录丢失、风控触发等问题。

**建议**：若主要使用小红书场景，在对应 Agent 上仅加载 xiaohongshu browseruse，从源头避免冲突。

若平台支持 per-agent 技能过滤（如 OpenClaw 的 `openclaw.json`），可配置示例：
  ...
  "agents": [
    {
      ...
      "skills": ["xiaohongshu"],
      "skillsExclude": ["browseruse"]
    }
  ]
}
```

本技能的 SKILL.md 已包含 Routing 提示，引导 Agent 在小红书场景下优先使用本技能；但平台级配置更可靠。
...
# 无头模式
uv run python scripts/chrome_launcher.py --headless
```
