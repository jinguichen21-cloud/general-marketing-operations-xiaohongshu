---
name: general-marketing-operations-xiaohongshu
description: |
...
- [ ] 已使用 `read_file` 读取对应子技能的 SKILL.md 文档（如 `skills/xhs-auth/SKILL.md`）
- [ ] 已识别用户意图对应的流程（如 xhs-auth 的流程 A/B/C/D）
- [ ] 已确认该流程的所有步骤、约束和参数要求
- [ ] 已确认是否需要执行登录检查（需浏览器操作的流程均需检查）
- [ ] 若需登录检查：已执行 `cli.py check-login` 并确认已登录
- [ ] 已检查前置条件（如用户确认等）
...
1. 启用本技能后，所有涉及到小红书登录、检查登录、切换账号等操作，都必须通过 `xhs-auth` 技能，不要使用其他的浏览器操作技能。
...
7. **素材管理**（"内容配图 / 管理素材 / 添加图片目录 / 搜索配图 / 同步素材 / 素材设置"）→ 执行 `xhs-material` 技能。
8. **内容规范**（"小红书怎么写 / 文案风格 / 创作规范 / 内容标准"）→ 执行 `xhs-content-rules` 技能。
9. **生成报告**（"生成报告"）→ 执行 `store-insight-report` 技能。
...
- 除认证外，所有操作前应确认登录状态（必须使用 `cli.py check-login` 来检测登录态，不允许使用内置浏览器打开页面）。
...
- 操作频率不宜过高，保持合理间隔。
- 涉及连接和操作浏览器时，严格遵循使用 scripts 下的 `chrome_launcher.py` 以及 `xhs/cdp.py` 的方式来连接和操作浏览器。
...
4. 前一步骤尚未完成且未进行状态检查之前（如登录流程中 send-code 之后才能 verify-code）
...
发表评论、回复、点赞、收藏。若用户未指定回复内容，可调用 `store-onboarding` -> `reply-kb` 技能从本地知识库检索相关话术，再基于话术生成回复内容。
